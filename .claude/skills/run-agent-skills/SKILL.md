...
description: Validate and drive the agentic-sdlc skills plugin. Use when asked to run or test this repo, validate the skills, or verify the plugin actually loads in Claude Code.
---
...
| step | what it proves |
|---|---|
| 1. `node scripts/validate.mjs` | repo invariants: manifests parse, frontmatter valid, no command or prompt surfaces, `skill:` refs + doc links resolve |
| 2. `claude plugin validate .` | marketplace manifest parses for the real consumer |
| 3. `claude --plugin-dir . plugin details agentic-sdlc` | the **local tree** registers every skill exactly once (`Source: agentic-sdlc@inline`) |
...
```bash
npm test   # alias for node scripts/validate.mjs — 9 groups, all must pass
```

Expected: `✓ All checks passed — 20 skills load correctly for every documented
agent.` (counts grow as skills are added; the five phase skills count among them).
