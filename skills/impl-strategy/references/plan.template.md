...
| Thomas | Verify this batch | `skill:thomas` (if enabled)   | ⛾     |

**Verify:** `<command>` → <what passing looks like; no regressions in related suite.>
**Thomas Gate (if enabled):** Dispatch `skill:thomas` as a subagent to execute every check in this batch's `Verify` line itself and confirm witnessed passing output. Thomas will also verify that all tracking-list rows for this batch are marked ✅ in `plan.md`. Mark the Thomas row ✅ only after Thomas issues an **APPROVED** verdict. If Thomas returns **NOT APPROVED**, the batch is not complete.
...
| Thomas | Full plan sign-off                                    | `skill:thomas` (if enabled)   | ⛾     |
