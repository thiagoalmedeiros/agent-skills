...
| `knowledge-base/` | Concept docs describing subsystems, architecture, setup | Any concept whose content describes the changed subsystem |
...
- **Subsystem/behavior changes** with an existing knowledge-base concept describing them → that concept
...
a contradiction resolved, dead content removed from within a file. Structural changes
(combine, move, delete, reorganize) require user confirmation first. How the bundle is
organized — frontmatter, naming, `index.md`, `log.md` — is not this skill's concern.

Deleting a **whole** bundle file is the one action that crosses that line: it strands the
file's `index.md` entry and leaves no changelog record, and repairing either is out of remit.
Delete a file only when an organization pass is scheduled to run after this skill; when none
is, list the deletion as a structural suggestion and leave the file in place.
